...
iPTF16geu \citep[][G17]{2017Sci...356..291G}, the subject of this work, is split into four resolved images by an individual galaxy. Furthermore, it was spectroscopically classified as a Type Ia supernova,  i.e., it is well-suited to precision estimates of the magnification caused by the lensing galaxy. 
...
The photometric data at hand is used to estimate the dimming by dust in the host galaxy, common to all four \sn images, and in the lensing galaxy for the individual images. This provides us with robust extinction measurements in the lensing elliptical galaxy at $z=0.216$ and we use the corrected lightcurves to measure the lensing magnification of the \sn images, as well as the time delays between them. The accurate \sn image positions are used to model the lens, as described in an accompanying paper (M\"ortsell et al, in prep). 
...
By comparing the model predictions of the flux ratios between \sn images with the reddening corrected observations, we assess the possibility that otherwise unaccounted  residuals are caused by lensing of substructures within the lensing galaxy, i.e., microlensing or millilensing.  
In another accompnaying paper (Johansson et al, in prep) we present that spectroscopic observations of \geu.
...
\ag{JOEL, RATIR data description missing...}
...
% we rebin  the NIRC2 \jband-band data to pixel scales similar the \wfcuvis and \wfcir data.  We then carry out model fits to the data, both 
% using and omitting the reference image, and can conclude that the background flux within a FWHM radius under the \sn images varies 
...
%Move this to the Johansson et al paper!
...
\section{Lightcurve fits}
We combine the ground based data with the resolved photometry from HST to fit for the global lightcurve parameters: lightcurve shape, color excess from either intrinsic color variations or dimming by dust in the host galaxy. In addition, we use the multi-band lightcurves of the resolved 
\sn images to fit for the lightcurve peaks and time offsets between the four \sn images, as well as extinction in the lensing galaxy for each individual line of sight. In total, 
...
In this section, we analyse the properties of the extinction of  iPTF16geu due to the dust in the host and the lens galaxies. The reddening parameters for our analysis include the $R_V$, $E(B-V)$ for the host galaxy and the $R_V$ and $E(B-V)$ for each image in the lens galaxy. 
We start with the simplifying assumption that the lens $R_V$ is the same for each SN image. Furthermore, we set the host and lens $R_V$ to the best fit value from the cosmological fits i.e. $R_V \sim \beta -1 \sim 2$ \citep[e.g.][]{2018ApJ...859..101S}.  The resulting parameters from the fit are summarised in Table~\ref{tab:params}.
...
The colour excess in the host galaxy is 0.322 $\pm$ 0.018 mag, which is assumed to be the same for each image since the difference in the light paths is very small for the images and hence, the dust properties are not expected to varying on those scales ($\sim$ few pc). For Images 1 and 2 we find a low value for the extinction of 0.19 $\pm$ 0.01 and 0.06, however, Images 3 and 4 have significant $E(B-V)$ of $\sim 0.7$ and $\sim  1.7$ mag. Setting the lens $R_V$ to 3.1, the canonical Milky Way value, we get lower $E(B-V)$ for each Image, as summarised in Table~\ref{tab:params}.

We also compute the total amplification for iPTF16geu and test the impact of our assumptions on the reddening parameters. For our fiducial case with both host and lens galaxy $R_V = 2$, we obtain an median amplification of 61.20 with a 68$\%$ credible region of (56.30, 64.96). Assuming the Milky Way value of $R_V = 3.1$ for both the host and the lens galaxy, we get a total amplification of 74.77 with a 68$\%$ credible region (66.27, 79.82). Leaving the $R_V$ free for the lens galaxy (keeping host galaxy $R_V$ fixed to 2), the total amplification is 59.28 and the 68$\%$ credible region is (55.02, 62.49). Hence, we find that the total magnification does not change more than $\sim 1 -1.5 \sigma$ for different assumptions on the $R_V$ in the host and lens galaxy. The individual image magnifications (Table~\ref{tab:params}) are also robust to the assumptions on the reddening properties.


%We test the impact of fixing the $R_V$ of the lens galaxy, firstly to the ensemble value for the SNe~Ia used in cosmological analysis, i.e. $R_V = 2$ and then to fixing it to the canonical Milky Way value of 3.1. We find that fixing the lens $R_V$ to higher than the best fit value results in lower inferred $E(B-V)$ values  for the SN images (see Table~\ref{tab:params}). The total amplification of the SN is 61.7 $\pm$ 5 and 71.4 $\pm$ 6 for the cases with $R_V$ fixed to 2 and 3, respectively, which are both consistent with the value obtained with lens $R_V$ as a free parameter {\bf SD: update numbers with higher resolution fit}. 
...
\begin{figure*}
    ...
    \caption{The figure shows the correlations between the amplification of each of the four images (normalised the median amplification of the faintest image, i.e. image 4) and reddening properties ($R_V$, $E(B-V)$). Images 1,2 and 3 have weak to no constraints on the $R_V$, however, for Image 4 we can place an upper limit on the $R_V$ of 3.2 at the 95$\%$ C.L. However, despite the weak constraints on the $R_V$, the amplification for each image is well-constrained. While Image 3 has a similar amplification to Image 4, Images 1 and 2 are $\sim$ 10 and $\sim$ 3 times brighter than Image 4, likely requiring an additional source of magnification.}
...
\subsubsection{Model-independent time-delays}
In addition to the model fit above, we also derive time-delays from the light curves without assuming an SN model. SNe~Ia have a distinct light curve morphology showing a second maximum $\sim$ few weeks after the first, we use the timing of this feature to derive constraints on $\Delta_t$. Since Images 1 and 2 are the brightest, the NIR light curves only have high enough signal to noise for these two images to derive a time-delay. We use two different methods to fit the data. Firstly, we derive $\Delta_{\rm t}$ using a Gaussian Process (GP) smoothing to the light curve. We use the Matern32 kernel implemented within the \texttt{GPy} package \citep{gpy2014}. We also use a cubic spline interpolation to compare with the GP results.

From our fits to the F110W  we derive time-delays of 2.54 $\pm$ 1.18 days and 0.37 $\pm$ 0.97 days which are consistent with zero time-delay and the inferred values from the model fits. 
\ag{NOT sure about this section. The obvious question is why only mention 110 and not e.g 160? Need to discuss caveats}

\section{Lensing from sub-structures}
\ag{Here we need an exectuve summary of Edvard's paper and present the case for why images 1 and 2
are deviant, afterextinction correction}

From Table~\ref{tab:params}, we report the values of the magnifications for each individual image. We find that Image 1 is the brightest and 4 is the faintest. Lens models of the system \citep[e.g.][M{\"o}rtsell et al. in prep.]{2017ApJ...835L..25M} suggest that the SN images would have very similar brightnesses since the SN images are symmetric around the lens. We test whether this could be a result of differential extinction, i.e. a difference in the $R_V$ for the dust in the region around Images 1 and 4. Assuming a different $R_V$ for Images 1 and 4, in this case setting it to extreme values of $R_V = 1$ and $R_V = 5$ for Images 1 and 4 respectively. We find that Image 4 is still fainter than Image 1 by a factor $\sim$ 6 and the best fit value compensates for the high input $R_V$ with a lower inferred $E(B-V)$. Hence, differential extinction is an unlikely explanation for the discrepancy between the observed and modelled image brightness ratios. Additionally, we fit for two different $R_V$'s in the lens galaxy, one for Image 1 and the other for Images 2,3,4. For this case, Image 4 is $\sim$ a factor 9 fainter than Image 1, since the $R_V$ for Image 4 is $1.2 \sim 1.3$ whereas the best fit $R_V$ of Image 1 $2.9 \sim 0.30$. The $R_V$ for Image 1 is  This conclusion is consistent with the symmetry of the system which would indicate that differential extinction is unlikely.
